...
def find_address_from_GPS(GPS):
    if not GPS['GPS_information']:
        ...
        if accres['error'] == 0:
            api = "http://api.map.baidu.com/reverse_geocoding/v3/?ak=&output=json&coordtype=wgs84ll&location={0},{1}".format(accres['baidu']['lat'],accres['baidu']['lng'])
            response = requests.get(api)
            content = response.text
            address = json.loads(content)
        else:
            api = "http://api.map.baidu.com/reverse_geocoding/v3/?ak=&output=json&coordtype=wgs84ll&location={0},{1}".format(lat, lng)
            response = requests.get(api)
            content = response.text
            address = json.loads(content)
